...
- **Smart accounts** are essentially smart contracts that are deployed onchain, while EOAs are created from key pairs

- **EOAs** are controlled with private keys with which they can initiate and sign transactions. Smart Accounts cannot initiate transactions on their own.

- **Smart Accounts** are programmable, they can enforce custom rules such as spending limits and automated actions. EOAs are not programmable.

<br />

Smart accounts replicate all of the functionality of EOAs and extend it through programmability.

<br />

Ethereum introduced **ERC-4337**, the standard for Account Abstraction, which defines how smart contracts interact.

<br />

Custom smart wallets are implementations of smart accounts that leverage their programmability. Custom smart wallets provide an interface for smart accounts the way wallet apps like MetaMask manage EOAs.

<br />

In the next section we comprehensively talk about how smart accounts work according to the ERC-4337 standards.
...
The ERC-4337 Account Abstraction standard is the foundation of modern smart wallets. It defines how smart accounts process transactions without relying on traditional externally owned accounts (EOAs). To understand how these wallets work, we first need to cover some supporting concepts.
...
**UserOperations (UserOps)**: In EOAs, users send transactions. In smart accounts, they send UserOperations. A UserOps is like a transaction envelope but with extra flexibility. For example, it includes a `callData` field that can batch multiple actions into one submission.
...
**Bundlers**: Unlike EOAs, UserOps aren't sent directly to the blockchain. Instead, they go to a bundler, a specialized node service. Bundlers collect many UserOps, bundle them into a transaction, and submit them to the EntryPoint contract. They play the same role as miners/validators do for regular transactions, but specifically for smart accounts.
...
It ensures that a submitted UserOperation is valid. The developer decides what "valid" means—it could be a simple check recovering an ECDSA signature or it could be a more advanced check that may require multiple signatures, biometrics and social recovery.
...
A frontend (wallet client/smart wallet) builds a UserOperation and signs it
...
The smart wallet is the user-facing client. Its job is to build UserOps, sign them, and send them to bundlers.

<br />

- **General-purpose wallets** (e.g., Coinbase Smart Wallet, Alchemy's smart wallet) aim to cover many use cases

- **Custom smart wallets** extend the basic functions of smart accounts with specialized rules or flows — for example, wallets for team-based spending, parental controls, or wallets that only work for gaming
...
- You can keep it simple by only validating an ECDSA signature

- You can make it more advanced with multisig checks, social recovery, or time-based restrictions

- You can design it for a specific use case like gaming, team spending, or delayed execution
